...
def parse_range(text: str) -> TimeRange:
    ...
    return TimeRange(start=start, end=end, label=text)
...
def ensure_ffmpeg_exists() -> None:
    if not shutil.which("ffmpeg"):
        raise UserError("找不到 ffmpeg，請先安裝後再執行")
    if not shutil.which("ffprobe"):
        raise UserError("找不到 ffprobe，請先安裝後再執行")
...
def run_ffmpeg(
    video_path: Path, rng: TimeRange, output_path: Path, verbose: bool
) -> None:
...
def main() -> int:
    ...
    try:
        ...
        video_duration = probe_duration(video_path) if args.check_duration else None
        ...
        for idx, rng in enumerate(ranges, start=1):
            ...
            run_ffmpeg(video_path, rng, video_out, args.verbose)
